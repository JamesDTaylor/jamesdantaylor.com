...
function createFlies() {
  return Array.from({ length: FLY_COUNT }, (_, i) => ({
    ...
    lagFactor: 0.018 + Math.random() * 0.014, // Individual inertia / reaction delay
...
export default function ParchmentQuillCursor() {
  // Opt-in activation: ONLY activated when an actual desktop mouse movement is detected on a desktop-sized screen
  ...
  // Global mouse move listener to activate ONLY for genuine desktop mouse input
  useEffect(() => {
    const handlePointerMove = (e) => {
      // If event is generated by touch (iOS/Android/iPad tap) or on a small mobile screen, ignore completely
  ...
  // Main animation frame loop for branch physics & buzzing flies
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      if (fliesCanvasRef.current) {
        fliesCanvasRef.current.width = window.innerWidth;
        fliesCanvasRef.current.height = window.innerHeight;
      }
    };
    handleResize();
    ...
    const tick = () => {
      ...
      // Decaying velocity when cursor stops
      p.vx *= 0.82;
      p.vy *= 0.82;

      // Natural drag lag & organic branch spring inertia
      const targetBranchTilt = -p.smoothVx * 0.55 + Math.sin(p.wavePhase) * 1.5;
      p.branchTiltVel += (targetBranchTilt - p.branchTilt) * 0.25;
      p.branchTiltVel *= 0.72;
      p.branchTilt += p.branchTiltVel;

      // Secondary kinetic leaf blade flexion
      const targetLeafTilt = -p.smoothVx * 0.85 + -p.smoothVy * 0.35 + Math.cos(p.wavePhase * 1.4) * 4.0;
      p.leafTiltVel += (targetLeafTilt - p.leafTilt) * 0.35;
      p.leafTiltVel *= 0.65;
      p.leafTilt += p.leafTiltVel;

      // Ambient breathing idle sway
      p.wavePhase += 0.05 + Math.min(0.2, p.speed * 0.01);

      // Apply 3D Spring Transforms
      if (branchContainerRef.current) {
        const hoverScale = isCardHovered ? 1.15 : isInteractive ? 1.08 : 1.0;
        const clickScale = isClicking ? 0.92 : 1.0;
        const finalScale = hoverScale * clickScale;
        branchContainerRef.current.style.transform = `scale(${finalScale}) rotate(${p.branchTilt.toFixed(2)}deg)`;
      }
      ...
      // ─── RENDER ORGANIC MIDGES / BUZZING FLIES SWARM ───
      const targetApexX = p.x;
      const targetApexY = p.y - 10;

      p.swarmX += (targetApexX - p.swarmX) * 0.016; // Organic chase delay
  ...
  }, [isDesktopActive, isCardHovered, isInteractive, isClicking]);

  // NEVER render on mobile or before desktop mouse is moved
  if (!isDesktopActive) {
    return null;
  }
      ...
      >
        <div
          ...
          style={{
            ...
            willChange: 'transform',
            filter: 'drop-shadow(0 2px 5px rgba(43, 27, 18, 0.35))',
            pointerEvents: 'none',
          }}
        >
          <svg
