...
/**
 * Pure Steampunk / Parchment Cursor & Buzzing Flies
 * Clean pointer arrow with buzzing flies lifecycle synced for frog predation.
 */
...
function createFlies(startX = 0, startY = 0) {
  return Array.from({ length: FLY_COUNT }, (_, i) => ({
    ...
    lagFactor: 0.018 + Math.random() * 0.014,
...
export default function SteampunkCursor() {
  ...
  useEffect(() => {
    const handlePointerMove = (e) => {
      ...
      // Initialize swarm near pointer on first move
      if (!p.initialized) {
        p.initialized = true;
        p.swarmX = clientX;
        p.swarmY = clientY - 8;
    ...
    const handleMouseDown = (e) => {
      ...
      // Scatter flies outward on click
      fliesRef.current.forEach((fly) => {
  ...
  // Animation Loop for Buzzing Flies
  useEffect(() => {
  ...
  return (
    <>
      ...
      <canvas
        ref={canvasRef}
        className="steampunk-sparks-canvas quill-flies-canvas"
        style={{
          position: 'fixed',
          inset: 0,
          pointerEvents: 'none',
      ...
      {/* Main Theme-Matched Clean Pointer Arrow */}
      <div
        ref={cursorRootRef}
